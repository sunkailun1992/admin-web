#!/usr/bin/env bash
# Admin Web secret warning scan: report suspicious credentials without rewriting files.
# Lockfiles are excluded because package integrity hashes are not application credentials and create noisy false positives.
set -euo pipefail

cd "$(git rev-parse --show-toplevel)"

excluded='^(scripts/check-secrets.sh|pnpm-lock.yaml|package-lock.json|yarn.lock)$'
