export default (initialState: { currentUser?: API.CurrentUser }) => {
  ...
  const canManageRagDocument =
    permissions.has('rag:document:manage') &&
    resourceCodes.has('menu:rag:document'); // 菜单和专用接口权限必须同时具备，接口仍由 RAG 服务二次鉴权。
  const canUseRagRetrieval = permissions.has('rag:retrieval:use');
  const canSeeRagGraph =
    permissions.has('rag:graph:view') && resourceCodes.has('menu:rag:graph');
  const canManageRagGraph = permissions.has('rag:graph:manage');
  ...
  return {
    ...
    canSeeRagDocument: canManageRagDocument,
    canUseRagRetrieval,
    canSeeRagGraph,
    canManageRagGraph,
  };
