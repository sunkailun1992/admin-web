...
- AI 新增或修改 TypeScript、TSX、JavaScript、CSS、配置、脚本、测试和示例等编程内容时，除 import、空行、单独的大括号和 Markdown 普通说明段落外，每一行新增或修改内容都要补充注释。
- 注释必须说明对应内容的用途、功能、业务含义、交互目的或框架衔接原因，不能只重复语法含义。
- 组件状态、请求参数、权限判断、表格列、表单项、提交转换、错误处理、上传下载、路由跳转和缓存读写必须逐行说明。
- 局部修改旧代码时，只要求本次改动行和相关逻辑补齐注释，不要为了补注释大面积重写无关代码。
